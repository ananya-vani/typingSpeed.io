...
const mistakeDisplay = document.querySelector(".mistake");
const wpmDisplay = document.querySelector(".wpm");
const cpmDisplay = document.querySelector(".cpm");

const welcomeView = document.querySelector(".welcome-view");
const typingTestView = document.querySelector(".typing-test-view");
...
const resultAccuracy = document.querySelector(".accuracy");
...
function loadParagraph() {
  ...
  paragraphElement.querySelector("span").classList.add("active");
  totalChars = paragraph.length;
}

function initTyping() {
  const chars = paragraphElement.querySelectorAll("span");
  ...
  if (charIndex === chars.length || timeLeft === 0) finishTest();
}
...
function showResult() {
  ...
  const accuracy =
    charIndex > 0 ? ((correctChars / charIndex) * 100).toFixed(2) : 0;

  resultAccuracy.innerText = accuracy;
  progressBar.style.width = `${accuracy}%`;

  updateLeaderboard(parseInt(wpmDisplay.innerText));
  const currentWPM = parseInt(wpmDisplay.innerText);
  const personalBest = localStorage.getItem("highestWPM") || 0;
  const personalMsgEl = document.querySelector(".personal-message");

  if (currentWPM > personalBest) {
    localStorage.setItem("highestWPM", currentWPM);
    personalMsgEl.textContent = "🎉 New Personal Best!";
  } else if (currentWPM == personalBest) {
    personalMsgEl.textContent = "⚡ You matched your Personal Best!";
  } else {
    const diff = personalBest - currentWPM;
    personalMsgEl.textContent = `🎯 You are ${diff} WPM away from your highest score.`;
  }
}

function updateLeaderboard(currentWpm) {
  document.querySelector(".current-wpm").textContent = `${currentWpm} WPM`;
  let storedHighScore = localStorage.getItem("highestWPM");

  if (!storedHighScore || currentWpm > parseInt(storedHighScore)) {
    storedHighScore = currentWpm;
    localStorage.setItem("highestWPM", storedHighScore);
  }

  document.querySelector(".highest-wpm").textContent = `${storedHighScore} WPM`;
}
